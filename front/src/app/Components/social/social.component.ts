...
interface Option {
  text: string;
  isSelected: boolean;
  score5: number; // Add the 'score' property to the Option interface
}
@Component({
  selector: 'app-social',
...
export class SocialComponent implements OnInit{
  ...
  ngOnInit() {
    this.esgService.getEsg(Categorie.Social, Partie.One).subscribe(
      (data: Esg) => {
        this.esg = data; // Assign the retrieved data to the 'esg' property

        // Now that 'esg' is populated, you can initialize 'options'
        this.options = [
          { text: this.esg.q1, isSelected: false, score5: 0 },
          { text: this.esg.q2, isSelected: false, score5: 1 },
          { text: this.esg.q3, isSelected: false, score5: 2 },
          { text: this.esg.q4, isSelected: false, score5: 3 },
          { text: this.esg.q5, isSelected: false, score5: 4 },
        ];
      },
      (error) => {
