...
interface Option {
  text: string;
  isSelected: boolean;
  score14: number; // Add the 'score' property to the Option interface
}
@Component({
  selector: 'app-gouvernance4',
  templateUrl: './gouvernance4.component.html',
  styleUrls: ['./gouvernance4.component.css']
})
export class Gouvernance4Component implements OnInit{
  esg: Esg = new Esg();
  ...
  ngOnInit() {
    this.esgService.getEsg(Categorie.Gouvernance, Partie.Five).subscribe(
      (data: Esg) => {
        this.esg = data; // Assign the retrieved data to the 'esg' property

        // Now that 'esg' is populated, you can initialize 'options'
        this.options = [
          { text: this.esg.q1, isSelected: false, score14: 0 },
          { text: this.esg.q2, isSelected: false, score14: 1 },
          { text: this.esg.q3, isSelected: false, score14: 2 },
          { text: this.esg.q4, isSelected: false, score14: 3 },
          { text: this.esg.q5, isSelected: false, score14: 4 },
        ];
      },
      (error) => {
  ...
  onDivClick(selectedOption: Option): void {
    ...
    this.vl=selectedOption.score14;
