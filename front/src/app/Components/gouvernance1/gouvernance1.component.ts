...
interface Option {
  text: string;
  isSelected: boolean;
  score11: number; // Add the 'score' property to the Option interface
}
@Component({
  selector: 'app-gouvernance1',
  templateUrl: './gouvernance1.component.html',
  styleUrls: ['./gouvernance1.component.css']
})
export class Gouvernance1Component implements OnInit{
  esg: Esg = new Esg();
 ...
 warnUser = false
  ...
  ngOnInit() {
    this.esgService.getEsg(Categorie.Gouvernance, Partie.Two).subscribe(
      (data: Esg) => {
        this.esg = data; // Assign the retrieved data to the 'esg' property

        // Now that 'esg' is populated, you can initialize 'options'
        this.options = [
          { text: this.esg.q1, isSelected: false, score11: 0 },
          { text: this.esg.q2, isSelected: false, score11: 1 },
          { text: this.esg.q3, isSelected: false, score11: 2 },
          { text: this.esg.q4, isSelected: false, score11: 3 },
          { text: this.esg.q5, isSelected: false, score11: 4 },
        ];
      },
      (error) => {
  ...
  onDivClick(selectedOption: Option): void {
    ...
    this.vl=selectedOption.score11;
