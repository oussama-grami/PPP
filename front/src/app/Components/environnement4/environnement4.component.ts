...
interface Option {
  text: string;
  isSelected: boolean;
  score4: number; // Add the 'score' property to the Option interface
  
}
@Component({
  selector: 'app-environnement4',
  templateUrl: './environnement4.component.html',
  styleUrls: ['./environnement4.component.css']
})
export class Environnement4Component implements OnInit{
  esg: Esg = new Esg();
  ...
  ngOnInit() {
    this.esgService.getEsg(Categorie.Environnement, Partie.Five).subscribe(
      (data: Esg) => {
        this.esg = data; // Assign the retrieved data to the 'esg' property

        // Now that 'esg' is populated, you can initialize 'options'
        this.options = [
          { text: this.esg.q1, isSelected: false, score4: 0 },
          { text: this.esg.q2, isSelected: false, score4: 1 },
          { text: this.esg.q3, isSelected: false, score4: 2 },
          { text: this.esg.q4, isSelected: false, score4: 3 },
          { text: this.esg.q5, isSelected: false, score4: 4 },
        ];
      },
      (error) => {
