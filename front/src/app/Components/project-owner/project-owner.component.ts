...
export class ProjectOwnerComponent {
  ...
  animateElements() {
    const formElements = document.querySelectorAll(
      '.form-floating, .radio-group, .file-upload'
    );
  ...
  onFileChange(event: any): void {
    ...
    if (this.pdfFiles.length + files.length > 5) {
      this.fileError = 'You can upload a maximum of 5 PDF files too many.';
      return;
    }
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (file.type !== 'application/pdf') {
        this.fileError =
          'Only PDF files are accepted. Please convert your documents to PDF format.';
        return;
      }

      // Check file size (10MB limit)
      if (file.size > 10 * 1024 * 1024) {
        this.fileError = `File "${file.name}" size exceeds the maximum limit of 10MB per file.`;
        return;
      }
  ...
  onSubmit() {
    ...
    // Validate the form and files
    if (this.projectForm?.valid && this.validateFiles()) {
      this.isSubmitting = true;

      // Prepare data for submission to match ProjectOwner entity
      ...
      console.log('Project Owner Data:', projectOwnerData);

      // Submit to backend with delay to show loading state
      setTimeout(() => {
        this.projectOwnerService.createProjectOwner(projectOwnerData).subscribe(
          (response) => {
            console.log('Project owner created successfully:', response);
          ...
          (error) => {
            ...
            this.submitError =
              'Failed to create project owner. Please try again.';
            this.isSubmitting = false;
      ...
      // Form or files are invalid
      this.validateFiles(); // Ensure file error message is set if files are invalid

      // Validate all form fields to show all errors
  ...
  highlightInvalidFields() {
    ...
    if (invalidElements.length > 0) {
      setTimeout(() => {
        (invalidElements[0] as HTMLElement).scrollIntoView({
          behavior: 'smooth',
          block: 'center',
        });
      }, 100);
