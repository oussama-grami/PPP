...
export class ProjectOwnerComponent {
  ...
  selectObjectiveImage(imageUrl: string, controlIndex: number) {
    ...
    if (this.selectedObjectiveImages.includes(imageUrl)) {
      ...
      this.selectedObjectiveImages = this.selectedObjectiveImages.filter(
        (img) => img !== imageUrl
      );

      // Find which form control has this image and clear it
      for (let i = 1; i <= 4; i++) {
        if (this.projectForm.get(`objectiveImage${i}`)?.value === imageUrl) {
          this.projectForm.get(`objectiveImage${i}`)?.setValue('');
          break;
        }
      }

      // Reorganize the remaining selected images to fill slots sequentially
      const remainingImages = [...this.selectedObjectiveImages];

      // Clear all form controls first
      for (let i = 1; i <= 4; i++) {
        this.projectForm.get(`objectiveImage${i}`)?.setValue('');
      }

      // Refill form controls with the remaining images
      remainingImages.forEach((img, idx) => {
        this.projectForm.get(`objectiveImage${idx + 1}`)?.setValue(img);
      });
    } else {
      // Check if already 4 objectives are selected
      if (this.selectedObjectiveImages.length >= 4) {
        return; // Don't add more than 4 images
      }

      // Add to selected list
      this.selectedObjectiveImages.push(imageUrl);

      // Find the first empty form control and set its value
      for (let i = 1; i <= 4; i++) {
        if (!this.projectForm.get(`objectiveImage${i}`)?.value) {
          this.projectForm.get(`objectiveImage${i}`)?.setValue(imageUrl);
          break;
        }
      }
    }
