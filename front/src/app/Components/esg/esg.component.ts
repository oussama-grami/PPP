...
export class ESGComponent implements OnInit{
  ...
  ngOnInit() {
    this.esgService.getEsg(Categorie.Environnement, Partie.One).subscribe(
      (data: Esg) => {
        this.esg = data; // Assign the retrieved data to the 'esg' property

        // Now that 'esg' is populated, you can initialize 'options'
        this.options = [
          { text: this.esg.q1, isSelected: false, score: 0 },
          { text: this.esg.q2, isSelected: false, score: 1 },
          { text: this.esg.q3, isSelected: false, score: 2 },
          { text: this.esg.q4, isSelected: false, score: 3 },
          { text: this.esg.q5, isSelected: false, score: 4 },
        ];
      },
      (error) => {
